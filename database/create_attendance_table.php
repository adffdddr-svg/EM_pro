 ...
 * إنشاء جدول الحضور والانصراف تلقائياً
 */
...
try {
    // قراءة ملف SQL
    $sql_file = __DIR__ . '/attendance_schema.sql';
    ...
    // تقسيم SQL إلى أوامر منفصلة (معالجة أفضل للأسطر المتعددة)
    $lines = explode("\n", $sql);
    $statements = [];
    $current_statement = '';
    ...
    foreach ($statements as $statement) {
        ...
        try {
            ...
            if (strpos($e->getMessage(), 'already exists') === false && 
                ...
                $success_count++; // نعتبره نجاح إذا كان موجوداً مسبقاً
            }
        }
    }
    
    echo "<!DOCTYPE html><html lang='ar' dir='rtl'><head><meta charset='UTF-8'><title>إنشاء جداول الحضور</title>";
    echo "<style>body{font-family:Arial;padding:20px;direction:rtl;} .success{color:green;} .error{color:red;}</style></head><body>";
    
    echo "<h2>✅ تم إنشاء جداول الحضور والانصراف!</h2>";
    ...
    echo "<p><a href='" . SITE_URL . "/admin/attendance/index.php' style='display:inline-block;padding:10px 20px;background:#667eea;color:white;text-decoration:none;border-radius:5px;'>➡️ الانتقال إلى صفحة الحضور</a></p>";
    echo "</body></html>";
